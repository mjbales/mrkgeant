  ...
  : fGlobalField(pEMfield)
{
  ...
  fDeltaOneStepCMD =
                 new G4UIcmdWithADoubleAndUnit("/MRK/field/setDeltaOneStep",this);
...
void MRKFieldMessenger::SetNewValue( G4UIcommand* command, G4String newValue)
{
  ...
  if( command == fLargestAcceptableStepCMD )
  {
    ...
    if(command==setCurrentFieldSettingsToModifyCmd){fGlobalField->setCurrentFieldSettingsToModify(setCurrentFieldSettingsToModifyCmd->GetNewIntValue(newValue));}
    if(command==setFieldPathCmd){fGlobalField->setFieldPath(newValue);}
    if(command==setFieldHistNameCmd){fGlobalField->setFieldHistName(newValue);}
    if(command==setFieldOffsetCmd){fGlobalField->setFieldOffset(setFieldOffsetCmd->GetNew3VectorValue(newValue));}
    if(command==setFieldExtentsCmd){fGlobalField->setFieldExtents(setFieldExtentsCmd->GetNew3VectorValue(newValue));}
    if(command==setFieldScalingValueCmd){fGlobalField->setFieldScalingValue(setFieldScalingValueCmd->GetNewDoubleValue(newValue));}
    if(command==setFieldSpaceDimCmd){fGlobalField->setFieldSpaceDim(setFieldSpaceDimCmd->GetNewIntValue(newValue));}
    if(command==setFieldFieldDimCmd){fGlobalField->setFieldFieldDim(setFieldFieldDimCmd->GetNewIntValue(newValue));}
    if(command==setFieldAngleXCmd){fGlobalField->setFieldAngleX(setFieldAngleXCmd->GetNewDoubleValue(newValue));}
    if(command==setFieldAngleYCmd){fGlobalField->setFieldAngleY(setFieldAngleYCmd->GetNewDoubleValue(newValue));}
    if(command==setFieldAngleZCmd){fGlobalField->setFieldAngleZ(setFieldAngleZCmd->GetNewDoubleValue(newValue));}
    if(command==setFieldIsMagneticCmd){fGlobalField->setFieldIsMagnetic(setFieldIsMagneticCmd->GetNewBoolValue(newValue));}
    if(command==setUseCubicInterpolationCmd){fGlobalField->setFieldUseCubicInterpolation(setUseCubicInterpolationCmd->GetNewBoolValue(newValue));}
    if(command==setFieldSymmetryXCmd){fGlobalField->setFieldSymmetryX(setFieldSymmetryXCmd->GetNewBoolValue(newValue));}
    if(command==setFieldSymmetryYCmd){fGlobalField->setFieldSymmetryY(setFieldSymmetryYCmd->GetNewBoolValue(newValue));}
    if(command==setFieldSymmetryZCmd){fGlobalField->setFieldSymmetryZ(setFieldSymmetryZCmd->GetNewBoolValue(newValue));}
    if(command==addNamedFieldsCmd){fGlobalField->addNamedField(newValue);}
