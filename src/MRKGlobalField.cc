...
//  fEquation = new G4EqEMFieldWithSpin(this);
	...
	G4TransportationManager* fTransportManager = G4TransportationManager::GetTransportationManager();
	...
	" MinStep=" << minStep << " DeltaChord=" << deltaChord << " DeltaOneStep=" << deltaOneStep << G4endl;
	G4cout << "                    " << " DeltaIntersection=" << deltaIntersection << " EpsMin=" << epsMin << " EpsMax=" << epsMax << G4endl<<
...
void MRKGlobalField::setupArray()
{
	...
	for (int i = 0; i < numElementFields; ++i)
		elementFieldArray[i] = (*theFieldList)[i];
}
...
void MRKGlobalField::addNamedField(string fieldSettingsName)
{
	...
	else if(fieldSettingsName == "SIMPLE_LIFETIME")
	{
